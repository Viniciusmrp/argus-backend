...
class ExerciseAnalyzer:
    ...
    def __init__(self):
        ...
        self.tracked_landmarks = [
            ...
            mp.solutions.pose.PoseLandmark.LEFT_THUMB, mp.solutions.pose.PoseLandmark.RIGHT_THUMB
        ]
...
class SquatAnalyzer(ExerciseAnalyzer):
    ...
    def analyze_frame(self, pose_results, frame_idx: int, fps: float, frame) -> Optional[Dict]:
        ...
        frame_data = {
            'frame_index': frame_idx, 'timestamp': frame_idx / fps,
    ...
    def get_final_analysis(self) -> Tuple[Dict, List[Dict]]:
        """Separates summary metrics from the detailed time series."""
        if not self.frame_metrics:
            return {}, []
        summary_metrics = {
            'status': 'success',
            'rep_counting': {'completed_reps': self.reps_completed},
            'metrics': {'total_frames_analyzed': len(self.frame_metrics)}
        }
        time_series_data = self.frame_metrics
        return summary_metrics, time_series_data
    ...
    def convert_numpy_types(self, obj):
        ...
        elif isinstance(obj, dict): return {k: self.convert_numpy_types(v) for k, v in obj.items()}
        elif isinstance(obj, list): return [self.convert_numpy_types(item) for item in obj]
        else: return obj